#include "platform-drv-dt-lock.h"
#include "platform.h"
...
int platform_drv_probe(struct platform_device*pdev) {
    ...
    pdrv_data.device_pcd = device_create(pdrv_data.class_pcd, dev, dev_data->dev_num, NULL, "pdev-lock-%d", pdrv_data.total_devices);
...
int platform_drv_remove(struct platform_device*pdev) {
    ...
    mutex_destroy(&dev_data->pcd_lock);
    return 0;
}
...
static int __init platform_drv_init(void) {
    ...
    int ret;
    ret = alloc_chrdev_region(&pdrv_data.dev_num_base, 0, MAX_DEVICES, "pdrv-lock");
    if(ret < 0) {
        pr_err("Alloc driver failed\n");
        return ret;
    }

    pdrv_data.class_pcd = class_create("pcd_lock_class");
...
MODULE_DESCRIPTION("Platform driver with sysfs + char dev (lock variant)");
